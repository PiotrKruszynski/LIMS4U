...
def validate_nip(tax_id):
    ...
    try:
        sum_products = sum(int(digit) * weight for digit, weight in zip(nip_digits[:9], weights))
    except ValueError:
...
def validate_exist_email(email):
    from .models import User # rozwiązuje problem cyklicznego importu validator -> user -> validator
...
def validate_confirm_passwords(password, confirm_password):
    if password != confirm_password:
        raise ValidationError(_("Hasła nie są takie same. Z validate_confirm_passwords"))
